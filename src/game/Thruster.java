...
public class Thruster{
	...
	public Thruster(int type, double posX, double posY, double posZ, double angleOffset, double zAngle,
		...
		this.type = type;
		this.scale = sqrt(thrust/(Main.options.thrustScale*Main.config.unitAccelMultiplier));
		...
		framesBetweenSwitch = Main.options.framesPerSec/55;
		
		double angle = angleOffset;
		if (direction == LEFT){
			angle += Utility.fixAngle(toDegrees(atan2(posX, -posY)) - 90);
	...
	public void draw(Graphics2D g, GameWindow window, Sprite sprite, boolean above, BufferedImageOp operation){
		...
		Image img = renderable[type].getImage(window.getRenderZoom()*scale, sprite.renderAngle+angle, true, (int)signum(sprite.getTurnSpeed()));
		...
		if (img != null && above == above(sprite.getAngle())){
			...
			if (translucent)
				g.setComposite(TRANSLUCENT_COMPOSITE);
	...
	private boolean above(double angle){
		...
		if (zOrder == ZOrder.BOTTOM)
			return false;
		return abs(Utility.fixAngle(angle + zAngle)) < 90;
	}
	
	public static void load(){
		renderable = new Renderable[numTypes];
		...
		for (int type = 0; type < numTypes; type++){
			...
			renderable[type] = new Renderable(1, UnitType.maxNumRenderAngles/2);
			renderable[type].load(new BufferedImage[]{image}, 1.0);
			...
			thrustSize[type] = renderable[type].getImage(1.0, 0.0, false, 0).getHeight(null);
		}
	}
}
